...
select Artist_id, Artist_name from artist a2 
where Artist_id not in (select Artist_ID from album_artist aa 
...
left join album_artist aa on aa.artist_id = a.artist_id
left join track t on t.album_id = aa.album_id
where track_duration in (select min(track_duration) from track t2
left join album_artist aa2 on aa.album_id = t2.album_id);
...
left join album a on a.album_id = t.album_id
group by a.album_id
having count(t.track_id) = (select count(t2.track_id) from track t2
